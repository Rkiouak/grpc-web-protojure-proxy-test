...
```lein new pedestal-service grpc-web-test```

The output of the pedestal-service template was
augmented in the following ways:

1. Added resources/addressbook.proto
2. Modified the src/grpc-web-test/service.clj to implement the GRPC service
...
2. Go to [localhost:8080](http://localhost:8080/) to see: `Hello from grpc-web-test, backed by Protojure Template! `
...
If you then update the `require` block of grpc-web-test/service.clj to use these generated clojure
...
cd resources/js
npm i
npx webpack client.js && rm -rf ../public/dist && mv dist ../public
```
## License
